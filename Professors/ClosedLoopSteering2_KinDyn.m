...
%Initial condition:
x=0; y=0; 
dotx=0; doty=0; phi=pi/2; dotphi=0; 
u=1; w=0; beta=0;

rocketCartState=[x;y;dotx;doty;phi;dotphi]; 
xSRef=[];
ySRef=[];
length(FTref)
for i=1:floor(length(FTref)/100),
   F1=0;
   F2=0;
   FT=FTref(i);
   psi=psiRef(i);
   S0=[x;y;dotx;doty;phi;dotphi];
   [tSol sol]=ode45(@ode2DRocketDyn,[(i-1)*dt i*dt],S0);
   tRec=[tRec;tSol];
   xSRef=[xSRef;sol(:,1)];
   ySRef=[ySRef;sol(:,2)];
   x=sol(end,1);y=sol(end,2);
   dotx=sol(end,3);doty=sol(end,4);
   phi=sol(end,5);dotphi=sol(end,6);   
   clear tSol;
   clear sol;
end
figure(6)
